...
function newRecord(gp, ym){
  ...
  return {id:rid(gp,ym), ym, mandal:g.mandal||u.mandal||'', gp,
    ...
    photos:[], sync:'local', photoFolder:'', updatedAt:'', attId:(DB.att[todayStr()]||{}).id||''};
}
...
function openFileSheet(){
  ...
  const g=$('#fileGo'); if(g) g.addEventListener('click', ()=>{ hideSheet(); syncOne(r); });
}
...
function renderRecords(){
  ...
  b.innerHTML = `<div class="group"><div class="card">` + rows.map(r=>{
    ...
    const badge = r.sync==='synced' ? '<span class="pill p-ok">Filed</span>' : '<span class="pill p-warn">On phone</span>';
...
async function syncOne(r){
  ...
  }catch(e){ toast(e.message||'Filing failed — it stays on this phone', 4500); }
...
async function syncAll(){
  ...
  try{
    ...
    const pend=Object.values(DB.records).filter(r=>r.sync!=='synced' || photoQueue(r).length);
